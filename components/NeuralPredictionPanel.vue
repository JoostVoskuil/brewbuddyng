<template>
  <section class="rounded border bg-background p-3 text-sm shadow-sm no-print">
    ...
    <div class="mb-3 grid grid-cols-2 gap-1 text-xs">
      <div
        v-for="feature in featureVector"
        :key="feature.key"
        class="flex justify-between gap-2 rounded bg-muted/40 px-2 py-1"
      >
    ...
    <label class="block text-xs font-medium">
      ...
      <select
        ...
        class="mt-1 w-full rounded border bg-background px-2 py-1"
        :disabled="loading"
      >
    ...
    <div class="mt-3 flex flex-wrap gap-2">
      ...
      <button
        type="button"
        class="rounded bg-primary px-3 py-1.5 text-xs text-primary-foreground disabled:opacity-60"
        :disabled="!selectedNetwork || predicting"
        @click="predict"
      >
    ...
    <div v-if="prediction" class="mt-3 rounded border p-2">
      <div
        v-for="(value, index) in prediction.outputs"
        :key="index"
...
const featureLookup = computed(() => {
  ...
  for (const feature of props.featureVector) {
    for (const key of [feature.key, feature.label, ...(feature.aliases ?? [])])
      entries.set(normalize(key), feature.value)
  }
  return entries
})
...
function inputsFor(network: NeuralNetworkItem): number[] | null {
  ...
  if (network.inputSize === props.featureVector.length)
    return props.featureVector.map((feature) => feature.value)
  return null
}

const matchingNetworks = computed(() =>
  networks.value.filter((network) => Boolean(network.weights) && inputsFor(network)),
)
const selectedNetwork = computed(
  () => matchingNetworks.value.find((network) => network.id === selectedId.value) ?? null,
)
...
async function predict() {
  ...
  try {
    const result = await $fetch<{ outputs: number[] }>(
      `/api/neural-networks/${network.id}/predict`,
      {
        method: 'POST',
        body: { inputs },
      },
