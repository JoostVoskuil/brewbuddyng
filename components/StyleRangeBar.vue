<template>
  <div class="space-y-1">
    ...
    <div class="space-y-1">
      <div class="relative h-5 rounded bg-muted overflow-hidden border" :aria-label="`${label} style range`">
        <div v-if="hasRange" class="absolute inset-y-1 rounded bg-green-500/35 dark:bg-green-400/30" :style="rangeStyle"></div>
        <div v-if="hasRange" class="absolute inset-y-0 w-px bg-foreground/70" :style="midpointStyle"></div>
      </div>
      <div class="relative h-5 rounded bg-muted overflow-hidden border" :aria-label="`${label} actual value`">
        <div v-if="hasRange" class="absolute inset-y-1 rounded bg-green-500/15" :style="rangeStyle"></div>
...
const status = computed<'withinRange' | 'nearRange' | 'outsideRange' | 'noRange'>(() => {
  ...
  return props.value >= min - tolerance && props.value <= max + tolerance ? 'nearRange' : 'outsideRange'
})
...
const midpointStyle = computed(() => ({ left: `${pct((Number(props.min) + Number(props.max)) / 2)}%` }))
