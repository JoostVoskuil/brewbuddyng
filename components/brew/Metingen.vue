    ...
    <div class="border rounded-lg p-4 space-y-3">
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm text-muted-foreground">{{ measurements.length }} {{ $t('brew.measurements').toLowerCase() }}</span>
          <button class="px-3 py-1.5 bg-primary text-primary-foreground rounded-md text-sm font-medium" @click="showForm = !showForm">
    ...
    <div v-if="showForm" class="border rounded-lg p-4 space-y-3">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <label class="text-xs font-medium text-muted-foreground">
          {{ $t('common.date') }}
          <input v-model="form.datetime" type="datetime-local" class="w-full mt-1 px-2 py-1.5 border rounded bg-background text-sm" />
        </label>
        <label class="text-xs font-medium text-muted-foreground">
          SG
          <input v-model.number="form.sg" type="number" step="0.001" min="0.990" max="1.200" class="w-full mt-1 px-2 py-1.5 border rounded bg-background text-sm font-mono" />
        </label>
        <label class="text-xs font-medium text-muted-foreground">
          T1 (°C)
          <input v-model.number="form.tempS1" type="number" step="0.1" class="w-full mt-1 px-2 py-1.5 border rounded bg-background text-sm font-mono" />
        </label>
        <label class="text-xs font-medium text-muted-foreground">
          T2 (°C)
          <input v-model.number="form.tempS2" type="number" step="0.1" class="w-full mt-1 px-2 py-1.5 border rounded bg-background text-sm font-mono" />
        </label>
        <label class="text-xs font-medium text-muted-foreground">
          Set (°C)
          <input v-model.number="form.setTemp" type="number" step="0.1" class="w-full mt-1 px-2 py-1.5 border rounded bg-background text-sm font-mono" />
        </label>
        <label class="col-span-2 md:col-span-3 text-xs font-medium text-muted-foreground">
          {{ $t('common.notes') }}
          <input v-model="form.notes" type="text" class="w-full mt-1 px-2 py-1.5 border rounded bg-background text-sm" />
        </label>
      </div>
      <div class="flex justify-end">
        <button class="px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium disabled:opacity-60" :disabled="saving" @click="submit">
...
const form = ref({ datetime: nowLocal(), sg: 1.000, tempS1: 0, tempS2: 0, setTemp: 0, notes: '' })
...
async function submit() {
  ...
  try {
    const body = {
      ...form.value,
      datetime: form.value.datetime ? new Date(form.value.datetime).toISOString() : new Date().toISOString(),
    }
    await $fetch(`/api/brews/${props.brewId}/measurements`, { method: 'POST', body })
    emit('add-measurement', body)
    showForm.value = false
    form.value = { datetime: nowLocal(), sg: 1.000, tempS1: 0, tempS2: 0, setTemp: 0, notes: '' }
  }
  finally {
    saving.value = false
  }
