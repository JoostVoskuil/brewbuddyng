    ...
    <div class="bg-background border rounded-lg shadow-xl w-full max-w-3xl p-4 space-y-4">
      ...
      <div class="space-y-2">
        <div
          ...
          :key="index"
          class="grid grid-cols-1 md:grid-cols-[1fr_7rem_1fr_auto] gap-2"
        >
          <input
            v-model="child.name"
      ...
      <div
        class="text-sm"
        :class="Math.abs(total - 1) < 0.001 ? 'text-green-700' : 'text-destructive'"
      >
      ...
      <div class="flex justify-between gap-2">
        ...
        <button
          ...
          :disabled="saving || Math.abs(total - 1) > 0.01"
          @click="divide"
        >
...
const total = computed(() =>
  children.value.reduce((sum, row) => sum + (Number(row.fraction) || 0), 0),
)
function add() {
  children.value.push({
    name: `${props.brewName} ${children.value.length + 1}`,
    fraction: 0,
    notes: '',
  })
}
async function divide() {
  ...
  error.value = ''
  try {
    await $fetch(`/api/brews/${props.brewId}/divide`, {
      method: 'POST',
