  ...
  <div class="water-treatment-print max-w-6xl space-y-6">
    <div class="flex items-start justify-between gap-4">
      ...
      <div class="flex gap-2 print:hidden">
        <button type="button" class="px-3 py-2 border rounded-md text-sm" @click="printSheet">{{ $t('common.print') }}</button>
        <NuxtLink class="text-sm underline" :to="`/recipes/${recipeId}`">{{ $t('common.back') }}</NuxtLink>
      </div>
    </div>
    ...
    <div v-else class="space-y-6">
      <section class="grid gap-4 md:grid-cols-2">
        <div class="rounded-lg border p-4 space-y-4">
          ...
          <select v-model.number="form.sourceWaterId" class="w-full px-3 py-2 border rounded-md bg-background" @change="copySelectedWater('source')">
        ...
        <div class="rounded-lg border p-4 space-y-4">
          ...
          <select v-model.number="form.targetWaterId" class="w-full px-3 py-2 border rounded-md bg-background" @change="copySelectedWater('target')">
            <option :value="null">{{ $t('water.treatment.custom') }}</option>
            <option v-for="water in waters" :key="water.id" :value="water.id">{{ water.name }}</option>
          </select>
          <WaterProfileInputs v-model="form.targetProfile" />
          <div class="flex flex-wrap gap-2">
            <button class="px-3 py-2 border rounded-md text-sm" :disabled="suggesting || !form.sourceWaterId || !form.targetWaterId" @click="suggestForTargetProfile">
      ...
      <section class="rounded-lg border p-4 space-y-4">
        <div class="flex flex-wrap gap-3">
          <label class="text-sm font-medium">{{ $t('water.treatment.mashVolume') }} (L)
            <input v-model.number="form.mashVolumeL" type="number" min="0" step="0.1" class="block w-32 mt-1 px-3 py-2 border rounded-md bg-background" />
          </label>
          <label class="text-sm font-medium">{{ $t('water.treatment.spargeVolume') }} (L)
            <input v-model.number="form.spargeVolumeL" type="number" min="0" step="0.1" class="block w-32 mt-1 px-3 py-2 border rounded-md bg-background" />
          </label>
        </div>
        ...
        <div class="flex flex-wrap items-end gap-2 border-t pt-4">
          <label class="text-sm font-medium">{{ $t('water.treatment.styleProfile') }}
            <select v-model.number="selectedStyleId" class="block min-w-56 mt-1 px-3 py-2 border rounded-md bg-background">
              <option :value="null">{{ $t('water.treatment.custom') }}</option>
              <option v-for="style in styles" :key="style.id" :value="style.id">{{ style.name }}</option>
            </select>
          </label>
          <button class="px-3 py-2 border rounded-md text-sm" :disabled="suggesting || !form.sourceWaterId || !selectedStyleId" @click="suggestForStyle">
      ...
      <section class="grid gap-4 lg:grid-cols-3">
        <div class="rounded-lg border p-4 space-y-3 lg:col-span-1">
          ...
          <label v-for="acid in acidInputs" :key="acid.key" class="block text-sm font-medium">
            {{ acid.label }}
            <input v-model.number="form.acids[acid.key]" type="number" min="0" step="0.1" class="w-full mt-1 px-3 py-2 border rounded-md bg-background" />
          </label>
          <label class="block text-sm font-medium">{{ $t('water.treatment.targetMashPh') }}
            <input v-model.number="form.acids.targetMashPh" type="number" min="3.5" max="7" step="0.01" class="w-full mt-1 px-3 py-2 border rounded-md bg-background" />
          </label>
          <p class="text-sm font-semibold">{{ $t('water.treatment.lacticNeeded') }}: {{ lacticNeeded.toFixed(2) }} mL</p>
          <p class="text-sm font-semibold">{{ $t('water.treatment.phosphoricNeeded') }}: {{ phosphoricNeeded.toFixed(2) }} mL</p>
          <div class="border-t pt-3 space-y-2">
            <h3 class="font-medium">{{ $t('water.treatment.spargeAcid') }}</h3>
            <label class="block text-sm font-medium">{{ $t('water.treatment.spargeAcidType') }}
              <select v-model="spargeAcidType" class="w-full mt-1 px-3 py-2 border rounded-md bg-background">
            ...
            <label class="block text-sm font-medium">{{ $t('water.treatment.targetSpargePh') }}
              <input v-model.number="spargeTargetPh" type="number" min="4" max="7" step="0.01" class="w-full mt-1 px-3 py-2 border rounded-md bg-background" />
            </label>
            <p class="text-sm font-semibold">{{ $t('water.treatment.spargeAcidNeeded') }}: {{ spargeAcidDisplay }}</p>
          </div>
        </div>
        ...
        <div class="rounded-lg border p-4 space-y-4 lg:col-span-2">
          ...
          <div class="grid gap-3 text-sm md:grid-cols-3">
            ...
            <p>{{ $t('water.treatment.residualAlkalinity') }}: {{ residualAlkalinity.toFixed(1) }} mEq/L</p>
            <p :class="ionBalanceClass">{{ $t('water.treatment.ionBalance') }}: {{ ionBalance.toFixed(1) }}%</p>
          </div>
          <div class="grid gap-3 md:grid-cols-2">
            <div class="rounded-md bg-muted/30 p-3">
              <p class="text-lg font-semibold" :class="mashPhClass">{{ $t('water.treatment.predictedMashPh') }}: {{ predictedMashPh.toFixed(2) }}</p>
              <p class="text-sm">{{ $t('water.treatment.mashPhTargetBand') }}</p>
            </div>
            <div class="rounded-md bg-muted/30 p-3">
              <p class="text-lg font-semibold">{{ $t('water.treatment.clSo4Ratio') }}: {{ clSo4Display }}</p>
      ...
      <div class="flex items-center gap-3">
        <button class="px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium" :disabled="saving" @click="save">
...
interface WaterRow extends WaterProfile { id: number; name: string; isDefault?: boolean }
interface StyleRow { id: number; name: string }
interface WaterProfile { calcium: number; magnesium: number; sodium: number; chloride: number; sulfate: number; bicarbonate: number; ph: number }
...
interface GristItem { amountKg: number; colorEbc: number; grainType?: string; added?: string }
interface SuggestionResponse { salts: { CaCl2: number; CaSO4: number; MgSO4: number; NaCl: number; NaHCO3: number; CaCO3: number }; acid: { type: string; mL: number }; predictedMashPh: number; predictedClSo4: number; targetProfile?: WaterProfile }
...
const emptyProfile = (): WaterProfile => ({ calcium: 0, magnesium: 0, sodium: 0, chloride: 0, sulfate: 0, bicarbonate: 0, ph: 7 })
const emptyAdditions = (): Additions => ({ cacl2: 0, caso4: 0, mgso4: 0, nacl: 0, nahco3: 0, caco3: 0 })
...
const mashProfile = computed(() => applyAdditions(form.value.sourceProfile, form.value.mashAdditions, form.value.mashVolumeL))
const spargeProfile = computed(() => applyAdditions(form.value.sourceProfile, form.value.spargeAdditions, form.value.spargeVolumeL))
const blendedProfile = computed(() => blendProfiles(mashProfile.value, form.value.mashVolumeL, spargeProfile.value, form.value.spargeVolumeL))
...
const ionBalanceClass = computed(() => Math.abs(ionBalance.value) > 10 ? 'text-amber-700 font-semibold' : '')
const predictedMashPh = computed(() => solvePh(mashProfile.value, form.value.mashVolumeL, grist.value, form.value.acids))
const mashPhClass = computed(() => mashPhBand(predictedMashPh.value) === 'target' ? 'text-green-700' : 'text-amber-700')
const clSo4Ratio = computed(() => chlorideToSulfateRatio(blendedProfile.value))
const clSo4Display = computed(() => Number.isFinite(clSo4Ratio.value) ? clSo4Ratio.value.toFixed(2) : '∞')
...
const spargeAcid = computed(() => acidForSpargeWater(spargeProfile.value, form.value.spargeVolumeL, spargeTargetPh.value, spargeAcidType.value))
const spargeAcidDisplay = computed(() => spargeAcidType.value === 'acidMalt' ? `${spargeAcid.value.amountGrams.toFixed(1)} g` : `${spargeAcid.value.amountMl.toFixed(2)} mL`)
const lacticNeeded = computed(() => acidNeeded('lactic', mashProfile.value, form.value.mashVolumeL, grist.value, form.value.acids.targetMashPh))
const phosphoricNeeded = computed(() => acidNeeded('phosphoric', mashProfile.value, form.value.mashVolumeL, grist.value, form.value.acids.targetMashPh))
const warnings = computed(() => {
  const list: string[] = []
  if (Math.abs(ionBalance.value) > 10) list.push(t('water.treatment.warningIonBalance'))
  if (predictedMashPh.value < 5.2 || predictedMashPh.value > 5.6) list.push(t('water.treatment.warningMashPh'))
  if (blendedProfile.value.calcium < 40 || blendedProfile.value.calcium > 200) list.push(t('water.treatment.warningCalcium'))
  if (blendedProfile.value.chloride > 200 || blendedProfile.value.sulfate > 600) list.push(t('water.treatment.warningFlavorIons'))
  return list
})
...
async function save() {
  ...
  saved.value = false
  try {
    await $fetch(`/api/recipes/${recipeId.value}/water-treatment`, { method: 'PUT', body: form.value })
    saved.value = true
  } finally {
    saving.value = false
  }
...
function toProfile(water: WaterRow): WaterProfile {
  return { calcium: water.calcium ?? 0, magnesium: water.magnesium ?? 0, sodium: water.sodium ?? 0, chloride: water.chloride ?? 0, sulfate: water.sulfate ?? 0, bicarbonate: water.bicarbonate ?? 0, ph: water.ph ?? 7 }
}

function applyAdditions(source: WaterProfile, additions: Additions, volumeL: number): WaterProfile {
  if (volumeL <= 0) return { ...source }
  const ppm = (grams: number, fraction: number) => (Math.max(0, grams || 0) * fraction * 1000) / volumeL
  return {
    ...source,
    calcium: source.calcium + ppm(additions.cacl2, 40.078 / 110.98) + ppm(additions.caso4, 40.078 / 136.14) + ppm(additions.caco3, 40.078 / 100.09),
    magnesium: source.magnesium + ppm(additions.mgso4, 24.305 / 120.37),
    sodium: source.sodium + ppm(additions.nacl, 22.99 / 58.44) + ppm(additions.nahco3, 22.99 / 84.01),
    sulfate: source.sulfate + ppm(additions.caso4, 96.06 / 136.14) + ppm(additions.mgso4, 96.06 / 120.37),
    chloride: source.chloride + ppm(additions.cacl2, 70.9 / 110.98) + ppm(additions.nacl, 35.45 / 58.44),
    bicarbonate: source.bicarbonate + ppm(additions.nahco3, 61.02 / 84.01) + ppm(additions.caco3 / 3, 61.02 / 100.09),
  }
}
...
function blendProfiles(a: WaterProfile, aL: number, b: WaterProfile, bL: number): WaterProfile {
  ...
  if (total <= 0) return { ...a }
  const mix = (key: keyof WaterProfile) => (a[key] * Math.max(0, aL) + b[key] * Math.max(0, bL)) / total
  return { calcium: mix('calcium'), magnesium: mix('magnesium'), sodium: mix('sodium'), chloride: mix('chloride'), sulfate: mix('sulfate'), bicarbonate: mix('bicarbonate'), ph: mix('ph') }
}
...
function acidForSpargeWater(water: WaterProfile, volumeL: number, targetPH: number, acidType: SpargeAcidType) {
  const residual = ((water.bicarbonate / 61) * 50 - water.calcium / 1.4 - water.magnesium / 1.7) / 50
  const carbonateMeq = spargeCarbonateAcidMeq(water.bicarbonate, volumeL, targetPH)
  const hardnessCredit = Math.max(0, volumeL) * (water.calcium / 20.04 / 3.5 + water.magnesium / 12.15 / 7)
  ...
  return { amountMl: acidType === 'acidMalt' ? 0 : acidMeq / meqPerMl, amountGrams: acidType === 'acidMalt' ? acidMeq / 0.333 : 0 }
}
...
function calcFrac(ph: number, pk1: number, pk2: number, pk3: number) {
  ...
  return [(h * h * h) / denominator, (h * h * k1) / denominator, (h * k1 * k2) / denominator, (k1 * k2 * k3) / denominator]
}
...
function zra(water: WaterProfile, ph: number) {
  ...
  return ct * (-charge(ph) + charge(water.ph || 7)) - water.calcium / 20.039 / 3.5 - water.magnesium / 12.1525 / 7
}
function solvePh(water: WaterProfile, volumeL: number, bill: GristItem[], acids: typeof form.value.acids) {
  let ph = 5.4
...
function protonDeficitAt(water: WaterProfile, volumeL: number, bill: GristItem[], ph: number) {
  return zra(water, ph) * Math.max(0, volumeL) + bill.filter((g) => (g.added ?? 'Mash') === 'Mash').reduce((sum, g) => sum + buffer(g) * (ph - diPh(g)) * Math.max(0, g.amountKg), 0)
}
...
function acidMeq(acids: typeof form.value.acids, ph: number, bill: GristItem[]) {
  ...
  return acids.lactic88Ml * ((1.214 * 0.88 * 1000) / 90.08) * acidFraction(ph, 3.08, 20, 20) + acids.phosphoric75Ml * ((1.58 * 0.75 * 1000) / 98) * acidFraction(ph, 2.12, 7.2, 12.44) + acids.acidMaltPercent * kg * 6
}
...
function acidNeeded(acid: 'lactic' | 'phosphoric', water: WaterProfile, volumeL: number, bill: GristItem[], target: number) {
  const required = Math.max(0, protonDeficitAt(water, volumeL, bill, target))
  const perMl = acid === 'lactic' ? ((1.214 * 0.88 * 1000) / 90.08) * acidFraction(target, 3.08, 20, 20) : ((1.58 * 0.75 * 1000) / 98) * acidFraction(target, 2.12, 7.2, 12.44)
...
const WaterProfileInputs = defineComponent({
  ...
  setup(props, { emit }) {
    const update = (key: keyof WaterProfile, value: number) => emit('update:modelValue', { ...props.modelValue, [key]: Number(value) || 0 })
    return () => h('div', { class: 'grid grid-cols-2 md:grid-cols-4 gap-3' }, ions.map((ion) => h('label', { class: 'text-sm font-medium' }, [ion.label, h('input', { value: props.modelValue[ion.key], type: 'number', step: ion.key === 'ph' ? '0.1' : '1', class: 'w-full mt-1 px-3 py-2 border rounded-md bg-background', onInput: (event: Event) => update(ion.key, Number((event.target as HTMLInputElement).value)) })])))
  },
})

const SaltGrid = defineComponent({
  props: { modelValue: { type: Object as PropType<Additions>, required: true }, title: { type: String, required: true } },
  emits: ['update:modelValue'],
  setup(props, { emit }) {
    const update = (key: SaltKey, value: number) => emit('update:modelValue', { ...props.modelValue, [key]: Number(value) || 0 })
    return () => h('div', { class: 'space-y-2' }, [h('h3', { class: 'font-medium' }, props.title), h('div', { class: 'grid grid-cols-2 md:grid-cols-3 gap-3' }, salts.map((salt) => h('label', { class: 'text-sm font-medium' }, [salt.label, h('input', { value: props.modelValue[salt.key], type: 'number', min: '0', step: '0.1', class: 'w-full mt-1 px-3 py-2 border rounded-md bg-background', onInput: (event: Event) => update(salt.key, Number((event.target as HTMLInputElement).value)) })])))])
  },
})

const ProfileCard = defineComponent({
  props: { title: { type: String, required: true }, profile: { type: Object as PropType<WaterProfile>, required: true } },
  setup(props) {
    const keys: (keyof WaterProfile)[] = ['calcium', 'magnesium', 'sodium', 'sulfate', 'chloride', 'bicarbonate']
    return () => h('div', { class: 'rounded-md bg-muted/30 p-3' }, [h('h3', { class: 'font-medium mb-2' }, props.title), ...keys.map((key) => h('p', { class: 'text-sm font-mono' }, `${key}: ${props.profile[key].toFixed(0)}`))])
  },
})
</script>
