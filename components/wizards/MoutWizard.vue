    ...
    <div class="w-full max-w-2xl rounded-lg border bg-background p-4 shadow-xl">
      ...
      <div class="grid gap-4 py-4 md:grid-cols-2">
        <label class="space-y-1 text-sm font-medium">
          {{ $t('recipe.targetOg') }}
          <input v-model.number="targetOg" type="number" step="0.001" min="1" class="w-full rounded border bg-background px-3 py-2 text-right" />
        </label>
        <label v-for="cat in categories" :key="cat.key" class="space-y-1 text-sm font-medium md:col-span-2">
          ...
          <input v-model.number="proportions[cat.key]" type="range" min="0" max="100" step="1" class="w-full" />
        </label>
      </div>
...
const emit = defineEmits<{ close: []; apply: [mode: 'append' | 'replace', fermentables: ReturnType<typeof planMaltWizard>] }>()

const targetOg = ref(props.target || 1.05)
const proportions = reactive<Record<MaltWizardCategory, number>>({ base: 85, caramel: 10, roast: 2, specialty: 3 })
const categories = [
...
const preview = computed(() =>
  planMaltWizard({ targetOg: targetOg.value, batchSize: props.batchSize, efficiency: props.efficiency, proportions }),
)
