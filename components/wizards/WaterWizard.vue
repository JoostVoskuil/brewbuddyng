<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div
      class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg border bg-background p-4 shadow-xl"
    >
      ...
      <div class="grid gap-4 py-4 md:grid-cols-3">
        <label class="space-y-1 text-sm font-medium">
          ...
          <select v-model.number="sourceAId" class="w-full rounded border bg-background px-3 py-2">
            <option v-for="water in waterProfiles" :key="water.id" :value="water.id">
              {{ water.name }}
            </option>
          </select>
        </label>
        ...
        <label class="space-y-1 text-sm font-medium md:col-span-3">
          <span class="flex justify-between"
            ><span>{{ $t('wizard.water.sourceBlend') }}</span
            ><span>{{ sourceAPercent }}% / {{ 100 - sourceAPercent }}%</span></span
          >
          <input
            v-model.number="sourceAPercent"
            type="range"
            min="0"
            max="100"
            step="5"
      ...
      <div class="grid gap-3 md:grid-cols-6">
        <label v-for="ion in ions" :key="ion.key" class="space-y-1 text-sm font-medium">
          {{ ion.label }}
          <input
            v-model.number="target[ion.key]"
            type="number"
            step="1"
            min="0"
            class="w-full rounded border bg-background px-2 py-1 text-right"
          />
        </label>
      </div>
      ...
      <div class="mt-4 grid gap-4 lg:grid-cols-2">
        <div class="rounded border p-3">
          ...
          <div
            v-for="salt in salts"
            :key="salt.key"
            class="flex justify-between border-t py-1 text-sm first:border-t-0"
          >
            <span>{{ salt.label }}</span>
            <span class="font-mono"
              >{{ result.additions[salt.key].toFixed(2) }} {{ salt.unit }}</span
            >
          </div>
        </div>
        <div class="rounded border p-3">
          ...
          <div
            v-for="ion in ions"
            :key="ion.key"
            class="grid grid-cols-3 gap-2 border-t py-1 text-sm first:border-t-0"
          >
            <span>{{ ion.label }}</span>
            <span class="text-right font-mono">{{ result.finalProfile[ion.key].toFixed(0) }}</span>
            <span class="text-right font-mono text-muted-foreground"
              >Δ {{ result.deltas[ion.key].toFixed(0) }}</span
            >
          </div>
        </div>
      </div>
...
const target = reactive<WaterProfile>({
  calcium: 75,
  magnesium: 10,
  sodium: 25,
  chloride: 80,
...
const sourceA = computed(() =>
  toProfile(props.waterProfiles.find((water) => water.id === sourceAId.value)),
)
const sourceB = computed(() =>
  toProfile(props.waterProfiles.find((water) => water.id === sourceBId.value)),
)
const result = computed(() =>
  planWaterWizard({
    sourceA: sourceA.value,
    sourceB: sourceB.value,
    sourceAPercent: sourceAPercent.value,
    target,
    volumeL: volumeL.value,
  }),
)
