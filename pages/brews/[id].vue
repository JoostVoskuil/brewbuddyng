<template>
  <div v-if="brew" class="space-y-6">
    <div class="hidden print:flex items-center justify-between gap-4 pb-4 border-b">
      ...
      <img v-if="printBreweryLogo" :src="printBreweryLogo" :alt="printBreweryName" class="max-h-20 max-w-40 object-contain" />
    </div>

    <div class="flex flex-col gap-4 print:hidden">
      <div class="flex items-center justify-between gap-4">
        <div class="flex items-baseline gap-3 min-w-0">
          <span v-if="brew.code" class="font-mono text-sm px-2 py-0.5 rounded bg-muted text-muted-foreground">{{ brew.code }}</span>
          <h1 class="text-2xl font-bold truncate">{{ brew.name }}</h1>
        </div>
        <div class="flex items-center gap-2">
          <button class="px-3 py-2 border rounded-md text-sm" @click="showDivideDialog = true">{{ $t('brew.divide.button') }}</button>
          <button class="px-3 py-2 border rounded-md text-sm" @click="printBrew">{{ $t('common.print') }}</button>
          <NuxtLink to="/brews" class="px-3 py-2 border rounded-md text-sm">← {{ $t('common.back') }}</NuxtLink>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3 border rounded-lg p-4">
        <label class="text-sm text-muted-foreground">
          {{ $t('brew.code') }}
          <input v-model="headerForm.code" class="w-full mt-1 px-3 py-2 border rounded-md bg-background font-mono" />
        </label>
        <label class="text-sm text-muted-foreground">
          {{ $t('brew.date') }}
          <input v-model="headerForm.brewDate" type="date" class="w-full mt-1 px-3 py-2 border rounded-md bg-background" />
        </label>
        <label class="text-sm text-muted-foreground">
          {{ $t('brew.status') }}
          <select v-model="headerForm.status" class="w-full mt-1 px-3 py-2 border rounded-md bg-background">
        ...
        <div class="flex items-end">
          <button class="w-full px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium disabled:opacity-60" :disabled="saveStatus === 'saving'" @click="saveHeader">
            {{ saveLabel }}
          </button>
        </div>
      </div>
    ...
    <nav class="flex gap-2 overflow-x-auto border-b print:hidden" aria-label="Brew tabs">
      <button
        ...
        :class="activeTab === tab.id ? 'border-primary text-primary font-medium' : 'border-transparent text-muted-foreground hover:text-foreground'"
    ...
    <BrewMaischen v-else-if="activeTab === 'maischen'" :brew="brew" :recipe="recipe" @save="saveBrew" />
    <BrewBrouwdag v-else-if="activeTab === 'brouwdag'" :brew="brew" :recipe="recipe" @save="saveBrew" @add-measurement="addMeasurement" @refresh="refreshAll" />
    ...
    <div class="flex items-center gap-4 print:hidden">
      <button class="text-sm text-primary hover:underline" @click="copyToRecipe">{{ $t('recipe.copyToRecipe') }}</button>
    </div>
  </div>
...
import type { Brew, BrewChecklistItem, BrewMeasurement, BrewWithDetails, RecipeWithIngredients } from '~/types'

type BrewTab = 'recept' | 'maischen' | 'brouwdag' | 'checklist' | 'vergisting' | 'bottelen' | 'metingen' | 'notities'
...
const activeTab = computed<BrewTab>({
  get() {
    ...
    return typeof value === 'string' && tabIds.includes(value as BrewTab) ? (value as BrewTab) : 'recept'
  },
  set(tab) {
...
async function saveBrew(patch: BrewPatch) {
  ...
  await runSave(async () => {
    ...
    const { measurements: _measurements, yeastStarter: _yeastStarter, divisions: _divisions, mashSteps: _mashSteps, boilTime: _boilTime, checklist, ...data } = next
    await $fetch(`/api/brews/${id}`, { method: 'PUT', body: patch.checklist ? { ...data, checklist } : data })
    await loadBrew()
  })
}

async function saveHeader() {
  await saveBrew({ code: headerForm.code, brewDate: headerForm.brewDate, status: headerForm.status })
}
