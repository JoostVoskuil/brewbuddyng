...
export default defineEventHandler(async (event) => {
  ...
  if (Math.abs(total - 1) > 0.01)
    throw createError({ statusCode: 400, message: 'Fractions must add up to 1.00' })
  ...
  for (const child of body.children) {
    const inserted: Brew[] = await db
      ...
      .values({
        ...
        notes: [
          child.notes,
          child.additionalIngredients
            ? `additionalIngredients=${JSON.stringify(child.additionalIngredients)}`
            : '',
        ]
          .filter(Boolean)
          .join('\n'),
