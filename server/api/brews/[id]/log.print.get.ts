...
import { brews, recipes, brewMeasurements, brewChecklist, brewDivisions, brewYeastStarter, settings } from '~/server/db/schema'
...
export default defineEventHandler(async (event) => {
  ...
  const [recipe, measurements, checklist, divisions, starters, settingRows] = await Promise.all([
    brew.recipeId ? db.select().from(recipes).where(eq(recipes.id, brew.recipeId)).get() : Promise.resolve(undefined),
...
function num(v: unknown) { return typeof v === 'number' && Number.isFinite(v) && v !== 0 ? v.toFixed(v < 10 ? 3 : 1).replace(/\.0+$/,'') : '—' }
function fmt(v: string | null | undefined) { return v ? new Date(v).toLocaleString() : '—' }
function row(name: string, value: string | null | undefined) { return `<tr><td>${esc(name)}</td><td>${esc(fmt(value))}</td></tr>` }
