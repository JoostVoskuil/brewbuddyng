...
export default defineEventHandler(async (event) => {
  ...
  const options = await readValidatedBody(event, (value) =>
    neuralNetworkTrainRequest.parse(value ?? {}),
  )
  const db = useDB()
  ...
  const result = trainStoredNetwork(network, trainingRows, {
    ...
    onEpoch: options.collectHistory
      ? (epoch, rmse) => {
          if (epoch === 1 || epoch % 25 === 0 || rmse <= options.targetRmse)
            history.push({ epoch, rmse })
        }
      : undefined,
  })
