...
import { createNeuralNetwork, serializeNetwork, type ActivationName } from '~/server/utils/calculations/neural'
...
export default defineEventHandler(async (event) => {
  ...
  const inputParams = body.inputParams ?? Array.from({ length: inputSize }, (_, i) => `input${i + 1}`)
  const outputParams = body.outputParams ?? Array.from({ length: outputSize }, (_, i) => `output${i + 1}`)
