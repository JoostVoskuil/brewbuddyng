export type GrainType = 'Base' | 'Kilned' | 'Crystal' | 'Roast' | 'Sour' | 'Special' | 'No malt' | string
...
export const MASH_PH_SOURCE = 'BrouwHulp/BrewBuddy TWater.MashpH, TWater.ProtonDeficit and TFermentable.BufferCapacity'
...
export function maltBufferCapacityMeqKgPh(grist: MashPhGristItem): number {
  if (isFiniteNumber(grist.acidTo57) && isFiniteNumber(grist.distilledInfusionPh) && grist.distilledInfusionPh !== 5.7 && Math.abs(grist.acidTo57) > 0.1) {
...
export function estimatedDistilledInfusionPh(grist: MashPhGristItem): number {
  if (isFiniteNumber(grist.distilledInfusionPh) && grist.distilledInfusionPh > 0) return grist.distilledInfusionPh
  const ebc = Math.max(0, finite(grist.colorEbc))
  switch (normaliseGrainType(grist.grainType)) {
    case 'Crystal':
...
export function maltAcidRequiredMeq(grist: MashPhGristItem, targetPh: number): number {
  ...
  return maltBufferCapacityMeqKgPh(grist) * (targetPh - estimatedDistilledInfusionPh(grist)) * amountKg
}

export function acidPotentialMeqAtPh(acids: MashPhAcids | undefined, ph: number, gristKg: number): number {
  if (!acids) return 0
...
export function predictMashPh(input: MashPhInput): MashPhResult {
  ...
  return {
    ...
    acidDeficitToTargetMeq: round(Math.max(0, protonDeficit({ ...input, acids: undefined }, finite(input.water.ph, 5.4))), 3),
  }
}

export function acidMlForTargetPh(input: MashPhInput, targetPh: number, acid: 'lactic88' | 'phosphoric75' = 'lactic88'): number {
  const required = Math.max(0, protonDeficit({ ...input, acids: undefined }, targetPh))
  const fraction = acid === 'lactic88' ? acidChargeFraction(targetPh, 3.08, 20, 20) : acidChargeFraction(targetPh, 2.12, 7.2, 12.44)
...
function mashGrist(grist: MashPhGristItem[]): MashPhGristItem[] {
  return grist.filter((item) => Math.max(0, finite(item.amountKg)) > 0 && (item.added ?? 'Mash') === 'Mash')
}
...
function normaliseGrainType(type: GrainType | undefined): string {
  ...
  if (value.includes('roast') || value.includes('black') || value.includes('chocolate')) return 'Roast'
  if (value.includes('sour') || value.includes('acid')) return 'Sour'
  if (value.includes('kiln') || value.includes('munich') || value.includes('vienna')) return 'Kilned'
