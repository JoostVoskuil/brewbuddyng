...
import { acidForSpargeWater, calculateAdditions, chlorideToSulfateRatio, type WaterProfile } from './water'
...
const ZERO_ADDITIONS = { cacl2: 0, caso4: 0, mgso4: 0, nacl: 0, nahco3: 0, caco3: 0, hcl: 0, h3po4: 0, lacticAcid: 0 }

export function suggestSalts(input: WaterSuggestionInput): WaterSuggestionResult {
  const volumeL = Math.max(0.1, finite(input.mashVolumeL) + Math.max(0, finite(input.spargeVolumeL)))
  ...
  const grist = input.gristEbcDistribution?.length ? input.gristEbcDistribution : [{ amountKg: 5, colorEbc: 8, grainType: 'Base', added: 'Mash' }]
  const mashPh = predictMashPh({ water: current, mashVolumeL: Math.max(0.1, finite(input.mashVolumeL)), grist }).predictedPh
  const acidMl = acidForSpargeWater(input.sourceWater, Math.max(0, finite(input.spargeVolumeL)), 5.6, 'lactic88').amountMl
...
export function targetProfileForStyle(style: { name?: string | null; category?: string | null; ibuMin?: number | null; ibuMax?: number | null; colorMin?: number | null; colorMax?: number | null }): WaterProfile {
  ...
  if (text.includes('ipa') || text.includes('bitter') || text.includes('pale ale') || ibu >= 45) {
    return { calcium: 140, magnesium: 18, sodium: 25, chloride: 60, sulfate: 300, bicarbonate: 80, ph: 7 }
  }
  if (text.includes('stout') || text.includes('porter') || srm >= 25) {
    return { calcium: 90, magnesium: 8, sodium: 45, chloride: 70, sulfate: 80, bicarbonate: 180, ph: 7 }
  }
  if (text.includes('malt') || text.includes('bock') || text.includes('dubbel')) {
    return { calcium: 80, magnesium: 8, sodium: 25, chloride: 120, sulfate: 60, bicarbonate: 120, ph: 7 }
  }
  if (text.includes('pils') || text.includes('lager') || text.includes('wheat')) {
    return { calcium: 55, magnesium: 5, sodium: 10, chloride: 50, sulfate: 60, bicarbonate: 40, ph: 7 }
  }
  return { calcium: 75, magnesium: 8, sodium: 20, chloride: 80, sulfate: 100, bicarbonate: 90, ph: 7 }
}
...
function score(profile: WaterProfile, target: WaterProfile): number {
  ...
  return ionScore('calcium', 0.8) + ionScore('magnesium', 0.4) + ionScore('sodium', 0.5) + ionScore('chloride') + ionScore('sulfate') + ionScore('bicarbonate', 0.6)
}
